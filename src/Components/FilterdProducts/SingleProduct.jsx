...
const SingleProduct = () => {
  ...
  const produtSize = product[0].size ? product[0].size[0] : "";
  const produtColor = product[0].color[0];
  const [size, setSize]=useState(produtSize);
  const [color, setColor]= useState(produtColor);
  const {id} = useParams();
